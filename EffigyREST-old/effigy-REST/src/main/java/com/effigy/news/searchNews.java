...
//            rs = stmt.executeQuery("select  n.ID, n.label,n.src, date_format(n.as_of, '%a %b %D') as ARTICLE_DT , date_format(n.as_of,'%r') as ARTICLE_TM, n.ARTICLE_URL,NF.fav_ID as fav_ID from tradeview.news N left outer join tradeview.news_favourites NF on N.ID=NF.fav_id where n.as_of >= curdate() - " + range + " and n.label like '%" + keyword + "%' ORDER BY n.as_of DESC, n.article_dt desc, n.extracted_dt desc  limit 200");
            rs = stmt.executeQuery("select  n.ID, n.label,n.src, date_format(n.as_of, '%a %b %D') as ARTICLE_DT , date_format(n.as_of,'%r') as ARTICLE_TM, n.ARTICLE_URL,NFT1.fav_ID as fav_ID, DATEDIFF(CURDATE() , N.AS_OF) AS DAYS_OLD   from tradeview.news N FORCE INDEX (idx_news_AS_OF) left outer join ( select NFT.FAV_ID, max(NFT.added_dt), count(NFT.FAV_ID) as total  from TRADEVIEW.news_favourites NFT GROUP BY NFT.FAV_ID) NFT1 ON N.ID =NFT1.FAV_ID where N.as_of >= curdate() - INTERVAL " + range + " DAY and (N.label like '%" + keyword + "%' OR N.Section like  '%" + keyword + "%' OR N.Content like  '%" + keyword + "%')  ORDER BY N.as_of DESC, N.article_dt desc, N.extracted_dt desc  limit 200");
    ...
    public static JSONArray getKeywords(int kType) {
        int j =0;
        try {
            Class.forName("com.mysql.cj.jdbc.Driver").newInstance();
        } catch (Exception ex) {
        ...
        try {
            ...
            switch (kType) {
                case 0:
                    rs = stmt.executeQuery("select TRIM(K.keyword) as keyword,count(*) as Total ,max(k.search_dt) as max_dt from tradeview.news_keywords K where K.search_dt > DATE_SUB(curdate(), INTERVAL 20 DAY) group by K.keyword order by count(*) desc  limit 60");
                    break;
                case 1:
                    rs = stmt.executeQuery("select TRIM(K.keyword) as keyword,count(*) as Total ,max(k.search_dt) as max_dt from tradeview.news_keywords K where K.search_dt > DATE_SUB(curdate(), INTERVAL 3 DAY) group by K.keyword order by max_dt desc  limit 10");
                    break;
                default:
                    rs = stmt.executeQuery("select TRIM(K.keyword) as keyword,count(*) as Total ,max(k.search_dt) as max_dt from tradeview.news_keywords K where K.search_dt > DATE_SUB(curdate(), INTERVAL 20 DAY) group by K.keyword order by count(*) desc  limit 60");
    ...
    public static JSONArray getLNewsDetails(String keyword,int range) {
        ...
        try {
            stmt = conn.createStatement();
            rs = stmt.executeQuery("select  ID, label,src, date_format(as_of, '%a %b %D') as ARTICLE_DT , date_format(as_of,'%r') as ARTICLE_TM, ARTICLE_URL from tradeview.news where as_of >= curdate() - INTERVAL " + range + " DAY and label like '%" + keyword + "%' ORDER BY as_of DESC, article_dt desc, extracted_dt desc  limit 200");
            System.out.println("Finished DB call");
            // or alternatively, if you don't know ahead of time that
            // the query will be a SELECT...
            //if (stmt.execute("SELECT foo FROM bar")) {
