...
public class tradeView {
    ...
    public static JSONArray getCompanyProfile(String keyword) {
        ...
        try {
            stmt = conn.createStatement();
            rs = stmt.executeQuery(" select * from tradeview.company1 where companyprofile like '%"+ keyword  + "%'  order by convert(closingprice,DECIMAL) desc");

            System.out.println("Finished DB call Tradeview profile extract:" + keyword + "" );
            // or alternatively, if you don't know ahead of time that
            // the query will be a SELECT...
            //if (stmt.execute("SELECT foo FROM bar")) {
            //	rs = stmt.getResultSet();
